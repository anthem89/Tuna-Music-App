...
'ORIGINAL SOURCE CODE CREDITED TO: Xertz (HF), UID: 1602992, EMAIL: xertzstudio@gmail.com
'ORIGINAL SOURCE CODE CREATED: 11/5/2014
'SOURCE CODE HAS BEEN MODIFIED BY CHRISTOPHER YU
...
#Region "Declarations"
    Private _CloseEntireApp As Boolean = True
    ...
    Private _HoverColour As Color = Color.FromArgb(80, 80, 80)
...
#Region "Properties"
    ...
    <Category("Control")>
    Public Property CloseEntireApp As Boolean
        Get
            Return _CloseEntireApp
        End Get
        Set(value As Boolean)
            _CloseEntireApp = value
        End Set
    End Property
    ...
    Protected Overrides Sub onmouseclick(e As MouseEventArgs)
        CaptureMovement = False
        If _AllowMaximize = False Then
            'IF ALLOW MAXIMIZE IS SET TO FALSE

            'MyBase.OnMouseDown(e)
            If MouseXLoc > Width - 30 AndAlso MouseXLoc < Width AndAlso MouseYLoc < 26 Then
                If _CloseEntireApp Then
                    Try
                        MyBase.ParentForm.Close()
                        If MyBase.ParentForm Is Nothing Then 'Check if form close was canceled during Closing event. If cancled, do not terminate app.
                            Application.Exit()
                        End If
                    Catch ex As Exception
                        End
                ...
                Else
                    Try
                        MyBase.ParentForm.Close()
                        If MyBase.ParentForm IsNot Nothing Then Exit Sub 'Check if form close was canceled during Closing event. If cancled, do not continue.
                        Dim keepAppOpen As Boolean = False
                        For Each form As Form In Application.OpenForms
                            If form.Visible = True Then keepAppOpen = True
                        Next
                        If keepAppOpen = False Then Application.Exit() 'Kill all unmanaged resources / stray processes and exit the application
                        Exit Sub
                    Catch ex As Exception
                        'Console.WriteLine("Form close error")
                    End Try
                End If
            ElseIf MouseXLoc > Width - 60 AndAlso MouseXLoc < Width - 30 AndAlso MouseYLoc < 26 And _AllowMinimize Then
        ...
        Else
            'IF ALLOW MAXIMIZE IS SET TO TRUE
            'MyBase.OnMouseDown(e)
            If MouseXLoc > Width - 30 AndAlso MouseXLoc < Width AndAlso MouseYLoc < 26 Then
                If _CloseEntireApp Then
                    Try
                ...
                Else
                    Try
                        ...
                        For Each form As Form In Application.OpenForms
                            If form.Visible = True Then keepAppOpen = True
                        Next
                        If keepAppOpen = False Then Application.Exit()
                        Exit Sub
                    Catch ex As Exception
                        'Console.WriteLine("Form close error")
                    End Try
    ...
    Protected Overrides Sub OnMouseDown(e As MouseEventArgs)
        If _AllowMaximize = False Then
            'IF ALLOW MAXIMIZE IS SET TO FALSE
            If e.Button = Windows.Forms.MouseButtons.Left And New Rectangle(0, 0, Width - 50, MoveHeight).Contains(e.Location) Then
        ...
        ElseIf _AllowMaximize = True Then
            'IF ALLOW MAXIMIZE IS SET TO TRUE
            If e.Button = Windows.Forms.MouseButtons.Left And New Rectangle(0, 0, Width - 90, MoveHeight).Contains(e.Location) Then
                CaptureMovement = True
                MouseP = e.Location
    ...
    Protected Overrides Sub OnDoubleClick(e As EventArgs)
        If _AllowMaximize = True Then
            MyBase.OnDoubleClick(e)
            If MouseXLoc < Width - 90 AndAlso MouseYLoc < 26 Then
                If _AllowMaximize Then
                    Select Case FindForm.WindowState
                        Case FormWindowState.Maximized
                            FindForm.WindowState = FormWindowState.Normal
...
#Region "Draw Control"
    ...
    Private Sub VisualStudioContainer_Click(sender As Object, e As EventArgs) Handles Me.MouseDown, Me.MouseUp
        Try
            'Set bounds for when form is maximized on different resolution screens
            Me.ParentForm.MaximumSize = Screen.FromRectangle(Me.ParentForm.Bounds).WorkingArea.Size
            ...
            With G
                ...
                Select Case State
                    Case MouseState.Over
                        ...
                        If AllowMinimize = True Then 'Minimize hover paint
                            If MouseXLoc > Width - 60 AndAlso MouseXLoc < Width - 30 AndAlso MouseYLoc < 26 Then
                                .FillRectangle(New SolidBrush(_HoverColour), New Rectangle(Width - 60, 1, 30, 25))
                            End If
                        End If

                        If MouseXLoc > Width - 30 AndAlso MouseXLoc < Width AndAlso MouseYLoc < 26 Then 'Close hover paint
                ...
                If AllowMinimize = True Then
                    'Draw Minimize Button
                    .FillRectangle(New SolidBrush(_ControlBoxColours), Width - 49, 17, 8, 2) '
                End If
                ...
                If _ShowIcon Then
                    Select Case _IconStyle
                        Case __IconStyle.FormIcon
                            ...
                            iconImage = New Icon(FindForm.Icon, New Size(45, 45)).ToBitmap
                            .DrawImage(iconImage, 6, -3, 37, 37)
                            .DrawString(Text, _Font, New SolidBrush(_FontColour), New RectangleF(50, 0, Width - 110, 32), New StringFormat With {.LineAlignment = StringAlignment.Center, .Alignment = StringAlignment.Near})
                        Case __IconStyle.None
                            .DrawString(Text, _Font, New SolidBrush(_FontColour), New RectangleF(45, 0, Width - 110, 32), New StringFormat With {.LineAlignment = StringAlignment.Center, .Alignment = StringAlignment.Near})
                    End Select
                Else
        ...
        Else
            ...
            With G
                ...
                Select Case State
                    Case MouseState.Over
                        If MouseXLoc > Width - 30 AndAlso MouseXLoc < Width AndAlso MouseYLoc < 26 Then
                            .FillRectangle(New SolidBrush(_HoverColour), New Rectangle(Width - 30, 1, 29, 25)) 'Close hover paint
                        ElseIf MouseXLoc > Width - 60 AndAlso MouseXLoc < Width - 30 AndAlso MouseYLoc < 26 Then
                            .FillRectangle(New SolidBrush(_HoverColour), New Rectangle(Width - 60, 1, 30, 25)) 'Maximize hover paint
                        End If
                        If AllowMinimize = True Then
                            If MouseXLoc > Width - 90 AndAlso MouseXLoc < Width - 60 AndAlso MouseYLoc < 26 Then
                                .FillRectangle(New SolidBrush(_HoverColour), New Rectangle(Width - 90, 1, 30, 25)) 'Minimize hover paint
                            End If
                        End If
                End Select
                'Draw Close Button
                ...
                If AllowMinimize = True Then
                    'Draw Minimize Button
                    .DrawLine(New Pen(_ControlBoxColours, 2), Width - 79, 18, Width - 70, 18)
                End If
                'Draw Maximize Button
                If FindForm.WindowState = FormWindowState.Normal Then
                    .DrawLine(New Pen(_ControlBoxColours, 1.6), Width - 49, 18, Width - 40, 18)
                    .DrawLine(New Pen(_ControlBoxColours, 1.6), Width - 49, 18, Width - 49, 10)
                    .DrawLine(New Pen(_ControlBoxColours, 1.6), Width - 40, 18, Width - 40, 10)
                    .DrawLine(New Pen(_ControlBoxColours, 1.6), Width - 49, 10, Width - 40, 10)
                    .DrawLine(New Pen(_ControlBoxColours, 1.6), Width - 49, 11, Width - 40, 11)
                ElseIf FindForm.WindowState = FormWindowState.Maximized Then
                    .DrawLine(New Pen(_ControlBoxColours, 1.6), Width - 48, 16, Width - 39, 16)
                    .DrawLine(New Pen(_ControlBoxColours, 1.6), Width - 48, 16, Width - 48, 8)
                    .DrawLine(New Pen(_ControlBoxColours, 1.6), Width - 39, 16, Width - 39, 8)
                    .DrawLine(New Pen(_ControlBoxColours, 1.6), Width - 48, 8, Width - 39, 8)
                    .DrawLine(New Pen(_ControlBoxColours, 1.6), Width - 48, 9, Width - 39, 9)
                    .FillRectangle(New SolidBrush(_BaseColour), New Rectangle(Width - 51, 12, 9, 8))
                    .DrawLine(New Pen(_ControlBoxColours, 1.6), Width - 51, 20, Width - 42, 20)
                    .DrawLine(New Pen(_ControlBoxColours, 1.6), Width - 51, 20, Width - 51, 12)
                    .DrawLine(New Pen(_ControlBoxColours, 1.6), Width - 42, 20, Width - 42, 12)
                    .DrawLine(New Pen(_ControlBoxColours, 1.6), Width - 51, 12, Width - 42, 12)
                    .DrawLine(New Pen(_ControlBoxColours, 1.6), Width - 51, 13, Width - 42, 13)
                End If
                If _ShowIcon Then
                    Select Case _IconStyle
                        Case __IconStyle.FormIcon
            ...
            End With
        End If
        e.Graphics.DrawLine(New Pen(Color.FromArgb(30, 30, 30), 1), 0, 1, Width, 1)
...
#Region "Declarations"
    ...
    Private ShowThumb As Boolean
    Private ThumbPressed As Boolean
    Private _ThumbSize As Integer = 5
...
#Region "Declarations"
    ...
    Private _Value As Integer = 0
    Private _Maximum As Integer = 100
    Private _ShowPercentSign As Boolean = False
    Private _StartingAngle As Integer = 110
    Private _RotationAngle As Integer = 255
    Private _BaseThickness As Integer = 1
    Private _ProgressThickness As Integer = 1
    Private _Font
#End Region
...
#Region "Properties"
    ...
    Sub New()
        ...
        Me.Font = New Font("Segoe UI Light", 16)
    End Sub
    ...
    'Chris
    <Category("Control")>
    Public Property BaseThickness As Integer
        Get
    ...
    'Chris
    <Category("Control")>
    Public Property ShowPercentSign As Boolean
        Get
            Return _ShowPercentSign
        End Get
        Set(value As Boolean)
...
#Region "Draw Control"
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        ...
        Dim percentCharacter As String

        If _BaseThickness > _ProgressThickness Then
            Padding = _BaseThickness
        Else
            Padding = _ProgressThickness
        End If

        If _ShowPercentSign = True Then
            percentCharacter = "%"
        Else
            percentCharacter = ""
        End If
        ...
        With G
            ...
            Select Case _Value
                Case 0
                    .DrawArc(New Pen(New SolidBrush(_BorderColour), _BaseThickness + 6), CInt(3 / 2) + 1 + CInt(Padding / 2), CInt(3 / 2) + 1 + CInt(Padding / 2), Width - 3 - 4 - Padding, Height - 3 - 3 - Padding, _StartingAngle - 3, _RotationAngle + 5)
                    .DrawArc(New Pen(New SolidBrush(_BaseColour), _BaseThickness + 3), CInt(3 / 2) + 1 + CInt(Padding / 2), CInt(3 / 2) + 1 + CInt(Padding / 2), Width - 3 - 4 - Padding, Height - 3 - 3 - Padding, _StartingAngle, _RotationAngle)
                    .DrawString(CStr(_Value) & percentCharacter, _Font, New SolidBrush(_TextColour), New Point(CInt(Width / 2), CInt(Height / 2 - 1)), New StringFormat With {.Alignment = StringAlignment.Center, .LineAlignment = StringAlignment.Center})
                Case _Maximum
                    .DrawArc(New Pen(New SolidBrush(_BorderColour), _BaseThickness + 6), CInt(3 / 2) + 1 + CInt(Padding / 2), CInt(3 / 2) + 1 + CInt(Padding / 2), Width - 3 - 4 - Padding, Height - 3 - 3 - Padding, _StartingAngle - 3, _RotationAngle + 5)
                    .DrawArc(New Pen(New SolidBrush(_BaseColour), _BaseThickness + 3), CInt(3 / 2) + 1 + CInt(Padding / 2), CInt(3 / 2) + 1 + CInt(Padding / 2), Width - 3 - 4 - Padding, Height - 3 - 3 - Padding, _StartingAngle, _RotationAngle)
                    .DrawArc(New Pen(New SolidBrush(_ProgressColour), _ProgressThickness + 3), CInt(3 / 2) + 1 + CInt(Padding / 2), CInt(3 / 2) + 1 + CInt(Padding / 2), Width - 3 - 4 - Padding, Height - 3 - 3 - Padding, _StartingAngle, _RotationAngle)
                ...
                Case Else
                    .DrawArc(New Pen(New SolidBrush(_BorderColour), _BaseThickness + 6), CInt(3 / 2) + 1 + CInt(Padding / 2), CInt(3 / 2) + 1 + CInt(Padding / 2), Width - 3 - 4 - Padding, Height - 3 - 3 - Padding, _StartingAngle - 3, _RotationAngle + 5)
                    .DrawArc(New Pen(New SolidBrush(_BaseColour), _BaseThickness + 3), CInt(3 / 2) + 1 + CInt(Padding / 2), CInt(3 / 2) + 1 + CInt(Padding / 2), Width - 3 - 4 - Padding, Height - 3 - 3 - Padding, _StartingAngle, _RotationAngle)
                    .DrawArc(New Pen(New SolidBrush(_ProgressColour), _ProgressThickness + 3), CInt(3 / 2) + 1 + CInt(Padding / 2), CInt(3 / 2) + 1 + CInt(Padding / 2), Width - 3 - 4 - Padding, Height - 3 - 3 - Padding, _StartingAngle, CInt((_RotationAngle / _Maximum) * _Value))
                    .DrawString(CStr(_Value) & percentCharacter, _Font, New SolidBrush(_TextColour), New Point(CInt(Width / 2), CInt(Height / 2 - 1)), New StringFormat With {.Alignment = StringAlignment.Center, .LineAlignment = StringAlignment.Center})
            End Select
        End With
        MyBase.OnPaint(e)
...
#Region "Declarations"
    ...
    Private _BorderColour As Color = Color.White 'Color.FromArgb(2, 118, 196)
#End Region
...
#Region "Declarations"
    ...
    Dim clr3 As Color = Color.White
    Dim clr4 As Color = Color.White
...
#Region "Properties"
    ...
    Property Color1() As Color
        ...
        Set(ByVal value As Color)
            clr1 = value
            MyBase.BackgroundImage = Gradient2D(MyBase.ClientRectangle, clr1, clr2, clr3, clr4)
        End Set
    End Property
...
#Region "Draw Control"
    ...
    'Private Sub GradientRectangle_Paint(sender As Object, e As PaintEventArgs) Handles Me.Paint
    '    SetStyle(ControlStyles.AllPaintingInWmPaint Or ControlStyles.UserPaint Or
    '            ControlStyles.ResizeRedraw Or ControlStyles.OptimizedDoubleBuffer, True)
    '    Me.DoubleBuffered = True
    '    e.Graphics.DrawImage(Gradient2D(MyBase.ClientRectangle, clr1, clr2, clr3, clr4), 0, 0)

    'End Sub

#End Region

#Region "Gradient Function"

    Private Function Gradient2D(ByVal r As Rectangle, ByVal clr1 As Color, ByVal clr2 As Color, ByVal clr3 As Color, ByVal clr4 As Color) As Bitmap
        DoubleBuffered = True
        ...
        Using g As Graphics = Graphics.FromImage(bmp)
            ...
            For y As Integer = 0 To r.Height - 1
                ...
                Using pgb As PathGradientBrush = New PathGradientBrush(getCorners(r).ToArray())
    ...
    Public Function getCorners(ByVal r As RectangleF) As List(Of PointF)
    ...
    Public Shared Function medianColor(ByVal cols As List(Of Color)) As Color
        Dim c As Integer = cols.Count
        Return Color.FromArgb(cols.Sum(Function(x) x.A) / c, cols.Sum(Function(x) x.R) / c, cols.Sum(Function(x) x.G) / c, cols.Sum(Function(x) x.B) / c)
    End Function
...
Public Class Button_NoFocusCue
    Inherits System.Windows.Forms.Button

    Private _DisplayFocusCues As Boolean = True
...
Public Class LabelClickThrough
    'A label control that passes click events to the control beneath it
    Inherits Label
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        ...
        Using aBrush As Brush = New SolidBrush(_FontColor)
            e.Graphics.DrawString(Text, Font, aBrush, ClientRectangle, strFormat)
        End Using
